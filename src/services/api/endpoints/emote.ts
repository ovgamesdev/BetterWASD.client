...
const endpoint = {
  ...
  getSharedEmotes: (limit: number, offset: number, search: string) =>
    axios.get(`/v1/emote/shared?limit=${limit}&offset=${offset}&query=${search}`),
  ...
  updateEmote: (id: any, data: object, user_id: number) =>
    axios.put(`/v1/emote/${id}/${typeof user_id !== "undefined" ? user_id : ""}`, data),
  ...
  updateAlias: (id: any, alias: string, user_id: number) =>
    axios.put(`/v1/emote/${id}/alias/${typeof user_id !== "undefined" ? user_id : ""}`, { alias: alias }),
